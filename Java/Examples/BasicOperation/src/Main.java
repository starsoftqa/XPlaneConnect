...
public class Main
{
    public static void main(String[] args)
    {
        ...
        try(XPlaneConnect xpc = new XPlaneConnect("192.168.0.102", 48001, 48000, 1000))
        {
            // Ensure connection established.
            xpc.getDREF("sim/test/test_float");

            System.out.println("Checking player aircraft position: Lat:" + xpc.getPOSI(0)[0] + " Lon: " + xpc.getPOSI(0)[1] + " Alt: " + xpc.getPOSI(0)[2]);
...
//
            ...
            xpc.sendDREF("sim/cockpit/switches/gear_handle_status", 0);
            ...
            {
                "sim/cockpit/switches/gear_handle_s" +
                        "tatus",
                "sim/operation/override/override_planepath"
            };
            float[][] results = xpc.getDREFs(drefs);
            if(results.length > 0 && results[0].length > 0 && results[0][0] == 0)
            {
